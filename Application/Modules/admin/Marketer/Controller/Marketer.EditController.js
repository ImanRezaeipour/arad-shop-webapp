var controller = function (scope, stateParams, state, service, toastr, upload, filter) {
    ...
    service.getCities().then(function (response) {
        ...
        service.getMarketerById({ id: stateParams.id }).then(function (response) {
            ...
            self.user.imageUrl = fileManagerGetUri + "/" + response.image;
            self.user.documentsImages = [];
            var index = 0;

            angular.forEach(response.documents,
                function (item) {
                    self.user.documentsImages.push({
                        url: item,
                        imageUrl: fileManagerGetUri + "/" + item,
                        order: index
                    });
                    index++;
                });
            ...
            angular.forEach(self.provinces,
                function (province) {

                    angular.forEach(province.cities,
                        function (city) {
                            if (city.id == self.user.marketerAddress.cityId) {
                                self.user.province = province;
                                self.user.marketerAddress.city = city;
                                return false;
                            }
                        });
    ...
    self.editMarketer = function () {
        ...
        service.editMarketer(self.user).then(function (response) {
...
angular.module('admin.MarketerApp').controller('admin.Marketer.EditController', controller)