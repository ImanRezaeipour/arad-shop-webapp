var service = function ($rootScope, $http, $q, $interval, toastr) {
  ...
  var httpConfig = function (server, method, api, params, data, isQueryString) {
    ...
    $http({
      ...
      if (error.status == 401) {
        ...
        toastr.error("لطفا مجددا وارد شوید", "عدم احراز هویت !");
      } else {
        console.log(error);
        
        $rootScope.loadingMain = false;
        if (error.data != null) {
          if (angular.isUndefinedOrNull(error.data.Errors)) {
            if (angular.isUndefinedOrNull(error.data.message)) {
          ...
          var message = ``;
          if (!angular.isUndefinedOrNull(error.data.Errors)) {
            angular.forEach(error.data.Errors,
              function (item) {
                
                errorComment.push(item);
              });
            errorComment.push(error.data.Message);
            message = '<div> <ul>';
            angular.forEach(errorComment,
              function (parameters) {
                message = message + "<li> <small> <em>" + parameters + "</em> </small> </li>";
              });
  ...
  this.patch = function (server, command, api) {

    return httpConfig(server, "PATCH", api, null, command, false);
  }
...
service.$inject = ["$rootScope", "$http", "$q", "$interval", "toastr"];
angular.module('HttpRestModule', []).service('RestService', service);
